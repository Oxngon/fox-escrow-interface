...
import Jewel from "../../contracts/Jewel";
...
function CreatOfferModal(props) {
    ...
    const [tokenAddress, setTokenAdddress] = useState(lockedTokens[0]);
    ...
    const resetState = () => {
        ...
        setTokenAdddress(lockedTokens[0].address);
    ...
    const fundSubmit = async () => {
        ...
        setFundLoading(true);
        try {
            const jewel = new Jewel(tokenAddress.address, library.getSigner());
            await jewel.transferAll(offerAddress.current);
            resetState();
            props.onHide(true);
        } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        setAndPing(!ethers.utils.isAddress(tokenAddress.address), setTokenAdddressERR);
        ...
        try {
            ...
            const tx = await offerFactory.createOffer(tokenAddress.address, stableCoin, amountOfStable);
    ...
    useEffect(async () => {
        if (!ethers.utils.isAddress(tokenAddress.address))
            return;
        const jewelToken = new Jewel(tokenAddress.address);
                    ...
                    <form>
                        <div className="offer-coin-div">
                            ...
                            <div className="set-coin">
                                ...
                                <select disabled={offerCreated} className="coin-balance select-font" onClick={(e) => setTokenAdddress(JSON.parse(e.target.value))}>
                        ...
                        <div className="offer-coin-div">
                            <div className="mb-1 ms-1">${tokenAddress.symbol} balance: {balance} </div>
                            <div className="set-coin">
